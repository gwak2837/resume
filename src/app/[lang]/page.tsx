...
const mangaViewerCards = [
  {
    ...
    items: [
      {
        label: '렌더링 성능',
        text: '페이지 모드와 스크롤 모드를 분리하고 react-window 기반 가상화와 프리패치 범위를 조정해 대용량 이미지 감상 시 초기 렌더 비용과 메모리 사용량을 함께 제어함',
      },
      {
        label: '상호작용 모델',
        text: 'Zustand 스토어로 이미지 번호, 보기 방향, 밝기, 줌, 화면 맞춤, 저데이터 설정을 분리해 터치 제스처와 뷰어 상태 변화를 예측 가능한 모델로 관리함',
      },
      {
        label: '이어읽기 복원',
        text: '세션 스토리지와 서버 동기화를 함께 사용해 이어읽기, 마지막 위치 복원, 탭 종료·백그라운드 전환 시 keepalive flush까지 일관되게 처리함',
      },
    ],
  },
  {
    ...
    items: [
      {
        label: '서비스 경계',
        text: 'Hono 기반 API를 인증, 검색, 서재, 알림, 랭킹, 게시글 등 도메인별 라우트로 분리하고 공통 미들웨어 계층에서 request ID, CSRF, CORS, secure headers를 일괄 적용함',
      },
      {
        label: '캐시 계층화',
        text: '공개 데이터와 개인화 데이터를 명확히 분리하고 Browser·Next.js·Vercel·Cloudflare별 Cache-Control을 다르게 설계해 응답 속도와 인프라 비용을 함께 최적화함',
      },
      {
        label: '정합성',
        text: 'Drizzle + PostgreSQL 스키마 위에 트랜잭션, row-level lock, 커서 기반 다중 정렬 페이지네이션을 적용해 감상 기록 한도, 북마크 가져오기, 서재 이동, 알림 조건 중복 같은 정책 제약을 서버에서 보장함',
      },
    ],
  },
  {
    ...
    bulletClassName: 'text-purple-400',
    dotClassName: 'bg-purple-500',
    items: [
      {
        label: '인증 체계',
        text: 'JWT access/refresh 토큰 흐름 위에 WebAuthn 패스키 로그인, TOTP 2단계 인증, 복구 코드, 신뢰 브라우저 모델을 조합해 사용성과 보안 수준을 함께 높임',
      },
      {
        label: '권한 경계',
        text: '로그인 상태와 익명 인증 상태를 미들웨어에서 분리해 관리하고, RLS가 적용된 개인화 테이블과 공개 데이터를 명시적으로 분리해 접근 경계를 분명히 함',
      },
      {
        label: '남용 방지',
        text: 'Turnstile, 애플리케이션 레벨 rate limiting, Cloudflare rate limiting, CSP/HSTS 기반 보안 헤더를 겹겹이 적용해 로그인·프록시·민감 엔드포인트의 공격면을 축소함',
      },
    ],
  },
  {
    ...
    bulletClassName: 'text-orange-400',
    dotClassName: 'bg-orange-500',
    items: [
      {
        label: '외부 의존성 안정화',
        text: 'Vercel Edge Runtime에서 외부 AI 만화·웹툰 소스를 프록시하고 exponential backoff, circuit breaker, 오류 정규화, degraded response로 upstream 변동성을 흡수함',
      },
      {
        label: '배포 자동화',
        text: 'k3s + Argo CD + Vault + External Secrets + Cloudflare Tunnel 조합으로 셀프호스팅 환경을 GitOps 기반으로 관리하고 bootstrap 스크립트로 초기 구성 과정을 자동화함',
      },
      {
        label: '관측 가능성',
        text: 'OpenTelemetry, Sentry, Grafana, Loki, Tempo, Blackbox, PrometheusRule을 연결해 애플리케이션 오류와 로그·트레이스·대외 장애를 한 흐름에서 추적하는 운영 체계를 구성함',
      },
    ],
  },
] as const
    ...
    <main className="mx-auto max-w-[718px]">
      ...
      <section id="personal-project" className="mb-12">
        ...
        <div className="flex justify-between whitespace-nowrap gap-4 my-4">
          <h4>AI 만화 웹 뷰어</h4>
          <div>2026년 4월 1일 ~ 현재</div>
        </div>
        <div className="text-gray-700 leading-relaxed text-sm my-4">
          <p>
            기획과 디자인부터 Next.js 프론트엔드, Hono API 서버, Drizzle/PostgreSQL 데이터 모델,
            Vercel Edge 프록시, Cloudflare 보안·캐시 정책, k3s·Argo CD·Vault·Grafana 기반 셀프호스팅
            운영까지 직접 설계·구현한 만화 감상 플랫폼입니다. 단순히 뷰어 화면을 만드는 데서 끝나지
            않고, 감상 경험, 데이터 정합성, 보안, 배포 자동화, 운영 안정성을 하나의 제품 관점에서
            책임졌습니다.
          </p>
          <p className="mt-2">
            뷰어에서는 페이지/스크롤 모드, 한·두 쪽 보기, 방향 전환, 밝기·줌, 저데이터 모드,
            이어읽기 복원, 이미지 가상화 같은 사용자 경험과 성능 문제를 함께 풀었고, 플랫폼
            차원에서는 외부 소스 장애를 흡수하는 멀티소스 페일오버, 재시도·회로 차단기,
            WebAuthn·TOTP·익명인증, 공개/개인화 데이터별 캐시 전략,
            OpenTelemetry·Loki·Tempo·Blackbox 기반 관측 체계를 구축해 제품 완성도와 운영 신뢰성을
            함께 높였습니다.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {mangaViewerCards.map((card) => (
            <div className={`p-4 rounded-lg border ${card.borderClassName}`} key={card.title}>
              <h4 className="font-bold text-gray-900 mb-3 flex items-center gap-2">
                <span className={`size-1.5 rounded-full ${card.dotClassName}`}></span>
                {card.title}
              </h4>
              <ul className="space-y-2 text-sm text-gray-700">
                {card.items.map((item) => (
                  <li className="flex items-start" key={item.label}>
                    <span className={`${card.bulletClassName} mr-2`}>▸</span>
                    <span>
        ...
        <div className="print:!block">
              ...
              {[
                ...
                },
                {
                  ...
                  역할: (
                    <>
                      <a
                        href="https://www.instagram.com/be_myseason/"
                        target="_blank"
                        rel="noreferrer"
                      >
                ...
                },
                {
                  이름: (
                    <a
                      href="https://web.cau.ac.kr/energy/index.htm"
                      target="_blank"
                      rel="noreferrer"
                    >
                ...
                },
                {
                  이름: (
                    ...
                    >
                      ...
                      <Image
                        src="/images/researchlab-logo.jpg"
                        alt="리서치랩"
                        width="69"
                        height="20"
                      />
              ...
              {[
                {
                  ...
                  역할: (
                    <>
                      <a
                        href="/pdf/IITP_2021_summer_UAV_ground_scanning_system.pdf"
                        target="_blank"
                      >
          ...
          <section id="award" className="mb-12">
            ...
            <ul>
              ...
              <li>
                <div className="grid grid-cols-[5fr_3fr_1fr_2fr] gap-4 my-2">
                  <a
                    href="https://www.jungle.co.kr/contest/204985"
                    target="_blank"
                    rel="noreferrer"
                  >
                    <b>MMO 해커톤 2회</b>
              ...
              <li>
                <div className="grid grid-cols-[5fr_3fr_1fr_2fr] gap-4 my-2">
                  <a
                    href="https://event-us.kr/pmsolution/event/40979"
                    target="_blank"
                    rel="noreferrer"
                  >
                    <b>2021 AI 해커톤</b>
              ...
              <li>
                <div className="grid grid-cols-[5fr_3fr_1fr_2fr] gap-4 my-2">
                  <a
                    href="https://blog.naver.com/djsepc/222200468576"
                    target="_blank"
                    rel="noreferrer"
                  >
          ...
          <section id="about" className="my-12">
            ...
            <p className="my-2">
              ...
              창업팀, 토스뱅크(주) 계약직, 리디(주) 정규직 등에서 다양한 프로젝트를 수행하며
              서비스가 세상에 나오기 위한 전체 개발 주기를 경험했습니다. 이 과정에서 서비스의 성공은
              동료와의 협업과 서로의 성장에 기반한다는 사실을 느꼈습니다.
            </p>
            ...
            <p className="my-2">
              ...
              토스뱅크에서 Admin Developer로 근무하며 여신 심사·관리·운영 시스템의 화면 설계와
              개발을 담당하던 중, 금융 분야에 특화된 지식과 여신 도메인에 대한 이해 부족이라는
              문제에 직면했습니다. 단순한 개발 지식만으로는 장님이 코끼리 다리 만지는 방식과
              비슷하여, 금융 도메인 지식을 알지 못하면 복잡한 금융 시스템을 효율적으로 구축할 수
              없다는 것을 깨달았습니다.
            </p>
            <p className="my-2">
              이 문제를 해결하기 위해 먼저 금융 관련 규제와 법률을 조사했습니다. 개인정보보호법과
              은행법의 관련 조항을 검토하여 시스템이 준수해야 할 요건과 제한 사항을 파악했습니다.
              특히 어떤 정보가 필수적으로 마스킹되어야 하는지 확인하기 위해 금융 업계 출신의
              팀원들과 협업하여 실무적인 지식을 습득했습니다. 이 과정에서 주민등록번호와 핸드폰 번호
              등 그 자체만으로 고객을 특정할 수 있는 민감한 정보는 암호화하여 저장하고, 화면에
              표시될 때는 마스킹되어 보여지도록 구현했습니다.
            </p>
            <p className="my-2">
              또한 전월세보증금의 자금 흐름을 명확히 파악하기 위해 대출 프로세스에서 각
              주체--임대인, 임차인, 보증 기관, 금융 기관--간의 역할과 거래 구조를 분석했습니다. 이를
              통해 자금이 어떤 경로로 이동하며, 어떤 주체가 대출금을 어떻게 보증하고, 각 단계에서
              필요한 법적 요건과 컴플라이언스 사항이 무엇인지 이해했습니다. 그 후 전월세보증금
              대출의 승인 과정에서 필요한 문서와 정보를 효율적으로 처리할 수 있도록 루모스(내부 여신
              시스템) 워크플로우를 개선하고, 관련 법률에 따른 보고 및 감사 요구사항을 충족하도록
              데이터 관리 방안을 수립하여, 시스템 설계와 개발 과정에서 서비스의 보안성과 규제
              준수성을 높였습니다.
            </p>
            <p className="my-2">
              이러한 경험을 통해 난관에 부딪혔을 때 문제의 근본 원인을 분석하고, 필요한 지식을
              습득하며, 팀원들과의 협업을 통해 해결책을 모색하는 것이 중요하다는 것을 배웠습니다.
            </p>
            <h3 className="text-lg my-4 ">
            ...
            <p className="my-2">
              ...
              긍정적인 영향을 미치는 결과를 만들어낼 때, 그리고 어려운 문제를 해결하여 팀의 목표
              달성에 기여할 때 가장 큰 성취감을 느낍니다.
            </p>
            <p className="my-2">
              예를 들어, 리디에서 만화 e북 웹 뷰어를 출시한 경험이 있습니다. 이 프로젝트에서는
              사용자가 웹에서 직접 만화를 감상할 수 있도록 기존 뷰어를 개선했습니다. 출시 후 만화
              e북 판매 매출이 전월 대비 3.61% 증가했고, 만화 e북 신규 고객의 웹 뷰어 사용률이 5%로
              상승했습니다. 구체적으로 모바일과 PC 웹 뷰어의 전환율은 각각 8.5%p와 99%p 증가했으며,
              뷰어 이탈률은 5%p와 99%p 감소했습니다. 또한 뷰어 오픈 시간도 각각 91%와 99%
              단축되었습니다. 이러한 성과를 통해 제가 기여한 기술적 개선이 사용자 경험과 비즈니스
              성과에 직접적인 영향을 미쳤음을 확인할 수 있었고, 이는 큰 보람으로 다가왔습니다.
            </p>
            <p className="my-2">
              또한, 난관을 극복하고 새로운 지식을 습득하여 문제를 해결했을 때 성취감을 느낍니다.
              토스뱅크에서 여신 심사·관리·운영 시스템을 개발할 당시, 금융 분야의 전문 지식이
              부족하여 어려움이 있었습니다. 이를 해결하기 위해 금융 관련 규제와 법률을 적극적으로
              학습하고, 금융 업계 출신 팀원들과 협업하여 도메인 지식을 습득했습니다. 전월세보증금
              대출 프로세스에서 자금의 흐름과 각 주체의 역할을 명확히 이해한 후, 시스템에 이를
              정확하게 반영했습니다. 그 결과, 전월세보증금대출 관련 프로세스를 자동화하고 시스템
              효율성을 높여 개인 담보대출 부문에서 약 2,000억 원의 대출 잔액 증가에 기여할 수
              있었습니다. 금융 산업의 문제를 분석하고 해결하여 팀의 목표 달성에 이바지했다는 점에서
              큰 만족감을 얻었습니다.
            </p>
            <h3 className="text-lg my-4 ">
            ...
            <p className="my-2">
              저는 개인적인 성장과 팀의 목표 달성을 중시하는 프론트엔드 개발자입니다. 프론트엔드
              개발의 핵심은 사용자 경험 최적화와 고객 간 신뢰 구축에 있다고 믿습니다. 이는 화면에
              콘텐츠를 처음으로 표시하는 시점(First Contentful Paint, FCP)의 최적화와 같은 기술적
              세부사항부터 시작됩니다. 제가 신디 창업팀의 일원으로 '카페 특색을 담은 SNS형 디저트
              정보 공유 서비스' 프로젝트에 참여한 경험은 이러한 신념을 실현하는 과정이었습니다.
            </p>
            <p className="my-2">
              ...
              설문조사 및 인터뷰를 통해 얻은 고객의 요구사항과 다소 차이가 있었고 추가적인 성능
              개선도 필요했습니다. 이에 신디 개발팀장으로서 팀원의 강점에 맞는 역할을 할당하고
              프로젝트 방향을 설정했습니다. 저는 프론트엔드 개발에서 디자이너에게 사용자
              인터페이스에 대한 세밀한 피드백을 제공했습니다. 또한 백엔드 개발에서 서비스의 사용자
              경험을 개선하기 위해 필요한 데이터만 주고 받도록 해서 데이터 처리의 효율성을 높였고,{' '}
              <a
              ...
              설계에 기여했습니다. 이 프로젝트는 성공적으로 출시되어 월간 사용자 수(Monthly Active
              User, MAU) 5천 명을 달성하는 성과를 이루었습니다.
            </p>
            <p className="my-2">
              개발자는 개인의 파트뿐만 아니라 프로젝트 전체적인 운영에 시야를 갖춰야 합니다. 개발
              일정과 코드 유지보수, 기획 요소를 녹일 수 있는 부분까지 고려하여 최적화된 기능을
              구현하여야 만족도 높은 결과물을 산출할 수 있기에, 이 부분에서 책임감을 나타낼 것을
              약속드립니다.
            </p>
            <h3 className="text-lg my-4 ">
            ...
            <p className="my-2">
              개발자로서 이루고자 하는 것은 기술을 통해 일상생활에서 사람들이 겪는 불편함을
              해결하고, 실질적인 가치를 제공하는 것입니다. 리디와 토스뱅크에서의 웹 프론트엔드 개발
              경험을 바탕으로, 사용자에게 이익이 되면서 비즈니스 매출을 증대시키고 낭비되는 비용을
              줄일 수 있는 솔루션을 개발하는 데 집중하고자 합니다.
            </p>
            <p className="my-2">
              ...
              전달해야 하는 번거로움도 존재합니다. 이러한 문제를 해결하기 위해 의료 데이터의
              표준화와, 개인정보보호법과 의료법을 준수하는 안전한 공유 시스템을 구축하여 환자들의
              불필요한 불편을 줄이고자 합니다.
            </p>
            <p className="my-2">
              또한, 최근 빠르게 성장하고 있는 무인 매장 시장에서, 무인 매장을 운영하면서 빈번하게
              발생하는 도난 문제는 운영 효율성과 신뢰성에 큰 영향을 미칩니다. 이를 해결하기 위해
              영상 인식 기술과 센서 데이터를 활용한 실시간 모니터링 시스템을 개발하여 도난을
              예방하고, 점주들이 수동으로 CCTV 화면을 주 52시간 이상 모니터링하지 않고도, 청소 및
              매대 정리 등 하루에 1시간만으로도 운영할 수 있는 솔루션을 구현하여 무인 매장의 운영
              효율성을 높이고 싶습니다.{' '}
            </p>
            <p className="my-2">
              그밖에도 높은 공실률로 인해 빌딩 자산이 효율적으로 활용되지 못하고 가치가 낭비되는
              현상이 있습니다. 이에 대한 해결책으로, 빌딩의 사용 데이터를 분석하고 수요와 공급을
              매칭하는 플랫폼을 개발하여, 빈 공간을 커뮤니티 시설로 전환하는 등 자산의 가치를
              극대화할 수 있는 방안을 모색하고 싶습니다.
            </p>
            <h3 className="text-lg my-4 ">
            ...
            <p className="my-2">
              ...
              금융 기업에서 계약직으로 근무하면서 경험했습니다. 반면, 한 가지에 집중하는 경우
              부분적으로 놓치는 요소가 있었습니다. 하지만 다양한 프로젝트와 인턴 실무를 경험하며
              프로젝트의 전체적인 흐름과 업무 분배의 중요성을 깨달았습니다.
            </p>
            <p className="my-2">
              현재는 업무 분석에 우선순위를 두고, 자체적인 데드라인 설정을 통해 시간 관리에 집중하고
              있습니다. 또한, 해당 도메인의 전문적인 지식을 가지고 있는 팀원에게 지속적으로 업무에
              대한 피드백을 요청해 서비스에 대한 시야를 넓히고 있으며, 프로젝트 전체를 이해하는
              개발자로 성장하고 있습니다.
            </p>
          </section>
